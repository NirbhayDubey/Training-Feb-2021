...
export default function Course(props) {
  ...
  useEffect(() => {
    ...
    if (!token) {
      ...
      LoginService.getSingleUser(decoded.id)
        .then((res) => {
          if (res.data) {
            const user = res.data;
      ...
      CourseService.getTrainingPlan(cname, token)
        .then((res) => {
          if (res.data.length > 0) {
            ...
            const plan = {
              ...plans[0],
              tp_whattolearn: plans[0].tp_whattolearn.split("\n"),
              tp_practice: plans[0].tp_practice.split("\n"),
              tp_assignment: plans[0].tp_assignment.split("\n"),
            };
            setCplan(plan);
            setDetails({
      ...
      <div className="container course">
        ...
        <div className="row py-4 m-0">
          <div className="col-lg-9 leftbar">
            ...
            <div className="dataframe bg-white p-3">{details.selected}</div>
          </div>
            ...
            <div className="card h-100 pb-3">
              <figure className="p-1 d-flex justify-content-center align-items-center">
                ...
                <video
                  id="displayvid"
                  className="w-100"
                  src="https://res.cloudinary.com/tutorialradix/raw/upload/v1624598201/videos/Z_introduction_apdxxo.mp4"
                  controls
                  onPlay={() => {
